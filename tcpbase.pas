...
uses Classes, SysUtils, SSockets;
...
type
  ...
  {$IFDEF UNIX}
  { TTextUnixSocket }
  TTextUnixSocket = class(TUnixSocket)
  public
    function  RecvText : String;
    procedure SendText(const AText : String);
  end;
...
{$IFDEF UNIX}
{ TTextUnixSocket }
function TTextUnixSocket.RecvText : String;
var len, n_len, r : LongInt;
begin
  r := 0;
  len := 0;
...
procedure TTextUnixSocket.SendText(const AText: String);
var len, n_len, w : LongInt;
begin
  w := 0;
  len := Length(AText);
